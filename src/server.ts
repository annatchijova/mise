...
type Ingredient = { id: string; role: string; qty: number; unit: string };
type Step = { order: number; text: string; dur_s: number; timer?: boolean; depends_on?: number[] };
type Recipe = { id: string; title: string; minutes: number; serves: number; ingredients: Ingredient[]; steps: Step[] };

const recipes: Recipe[] = JSON.parse(readFileSync(new URL("../data/recipes.json", import.meta.url), "utf8"));
...
export function searchRecipes(input: { query?: string; use_ingredients?: string[]; max_minutes?: number }) {
  ...
  const candidates = recipes
    ...
    .map((r) => {
      ...
      return { id: r.id, title: r.title, minutes: r.minutes, serves: r.serves, have_pct: pct, missing: r.ingredients.filter((i) => !wanted.some((w) => i.id === w || i.id.includes(w))).map((i) => i.id) };
    })
...
httpServer.listen(PORT, () => {
  console.log(`mise MCP server listening on http://localhost:${PORT}/mcp`);
});
