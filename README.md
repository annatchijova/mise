...
- **Zero Waste / Desperdicio Cero.** `zero_waste` turns the pantry into a cooking lesson: existing recipes ranked by urgent ingredients, then by pantry coverage, each paired with a versioned technique lesson (browning, emulsifying, baking, simmering, raw, fermenting) in English and Spanish — a practice prompt and a plating exercise, only for a technique the matched recipe actually uses. For urgent ingredients that regrow from a scrap in water or soil (green onion, celery, garlic, ginger and others), a separate, ingredient-keyed tip rides alongside the recipe rather than being gated by it — regrowing is not a cooking technique. It never invents a recipe, a substitution, a lesson for an unsupported technique, or a regrow tip for an ingredient the table does not cover, and it does not claim a measured saving: economy means checking what is on hand before buying, ecology means using food before it is discarded, neither is quantified in money or emissions. See `src/zero_waste.ts`.
...
| **Zero waste** | `zero_waste` ranks existing recipes by urgent-then-soon pantry ingredients, then coverage, and pairs each match with a bilingual (EN/ES) technique lesson — a practice prompt and a plating exercise — only for a technique that recipe actually uses. It invents nothing and claims no measured saving in money or emissions: economy is checking what is on hand before buying, ecology is using it before it goes off. |
| **How long it keeps** | 151 curated shelf-life rows, derived at fold time and never stored. An estimate from the table and a date you gave stay two different things all the way to the sentence — "roughly four days by my reckoning" against "expiring today or tomorrow". |
| **Substitutions** | 90 curated rows keyed on (ingredient, role, technique), each with an integer ratio, what changes and what breaks. `substitute` walks a fallback chain and says which level it answered from; an ingredient the table does not cover gets silence, out loud. Swaps people actually make are queued for a curator and **never read back** — the table grows when a person edits it, and at no other moment. |
| **Cooking** | A pure state machine: ordered steps, parallel timers, dependencies, and a transition log. Eight `cook_*` tools, and a second person can take a track of their own — the scheduler reads the dependency graph and says who does what, or says plainly that this recipe is one long chain and cannot be split. "Pause" — a day later, on another device — "where was I?" answers step 5 with the timer where it stopped. Finishing deducts what was used, as `inferred`, honouring any swap you recorded. `cook_review` reads the log back: how long each step really took, what was done out of order, and which questions the record cannot answer. For a step nobody sits through — the corpus has a four-to-six week sauerkraut — `cook_checked` says what to look at on the way through, from a plan a person wrote for that step. |
| **The week** | `plan_week`: leftovers first — a portion in the fridge is a dinner that needs no cooking — then a deadline pass that schedules food on the last day it is still good, then a coverage pass, with a per-meal reason, a consolidated shopping list that counts shortfalls, and a plan hash. Time limits per day, a spending ceiling, and what the shopping costs. `plan_diff` says what changed since the last plan and why, quoting the planner rather than composing a story. |
| **Buying** | A demo grocery of 108 SKUs, `cart_from_plan` and `cart_edit`, and a UCP-shaped checkout — five endpoints, idempotency with 409, allergen disclosure, server-side tax and shipping. Completing writes back to the pantry, so the next plan already sees it. |
| **Views** | Four MCP Apps `ui://` resources — step card, weekly grid, cart, pantry — each one self-contained HTML with no network of its own. The step card puts the mise en place up as a checklist rather than a sentence, carries the book and locator the recipe came from, and has a large-print mode: one step, high contrast, nothing else. |
...
| **Receipts** | A pasted shop receipt through the same signed door, read by a written grammar rather than a model — because a receipt is evidence, and a parser with a grammar can be told it is wrong. Every line ends up an event, an unmapped name reported with its raw text, or a skipped line **with the rule that skipped it**. A receipt may only ever claim `inferred`: it proves the shop sold it, not that it reached your kitchen. |
...
`npm run check` runs the typecheck, 361 tests and nine data validators.
